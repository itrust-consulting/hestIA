...
class ITRXLSXParser(XLSXParser):
    """XLSXParser extended for iTrust document templates (Hist-sheet metadata)."""

    def __init__(self, file=None, selected_sheets: list[str] | None = None):
        super().__init__(file=file, selected_sheets=selected_sheets)

    def get_metadata_from_cover_page(self) -> dict:
        """
        Extracts metadata from the Hist-sheet.
        Expected layout:
          Row 0: Title
          Row 1: Subtitle
          Rows 3-8: key/value pairs (Reference, Version, ...)
        """
        cover = self.doc.get("Hist")
        if cover is None:
            raise ValidationError("No 'Hist' sheet found in XLSX document.")

        meta: dict = {}
        meta["title"] = self.clean_string(str(cover.iloc[0, 0]))
        meta["subject"] = self.clean_string(str(cover.iloc[1, 0]))

        for i in range(3, 9):
            try:
                key = self.clean_string(str(cover.iloc[i, 0]))
                if not key:
                    continue
                val = cover.iloc[i, 2]
                meta[self.normalize_key(key)] = val.strip() if isinstance(val, str) else val
            except IndexError:
                break
